...
# scatter plot with burgs per 1000 people
# predict function using 1000 people as pop and 
...
burgs_num_per_tract_real_pos$income_level <- with(burgs_num_per_tract_real_pos, factor(
                            findInterval(median_income, c(-Inf,
                               quantile(median_income, probs=c(0.25, .5, .75)), Inf)), 
                            labels=c("lowest","low","medium", "high")
      ))
...
unique(burgs_num_per_tract_real_pos[["income_level"]])

# burglary_census_combo_latlong
burgs_num_per_tract_real_pos
```

How can you interpret the meaning of the output? Lower income tracts have higher numbers of burglaries. The higher the income level the less likely it is to have as many burglaries. 
...
burg_median_income_lvl_lr <- glm(
  num_burglaries ~ income_level,
  data = burgs_num_per_tract_real_pos,
  family = 'poisson',
  offset=log(population)
)
summary(burg_median_income_lvl_lr)
...
burgs_num_per_tract_real_pos |>
    ...
    labs(x = "Number of Burglaries", y= "Frequency of census tracts with num of burgs", title = "")
...
hist(burgs_num_per_tract_real_pos$median_income) 
hist(burgs_num_per_tract_real_pos$num_burglaries) 
```
...
library(jtools)
effect_plot(burg_median_income_lr, pred = median_income, interval = TRUE, plot.points = TRUE, 
             jitter = 0.05)
...
plot_summs(burg_median_income_lvl_lr)

```

```{r}

plot_summs(burg_median_income_lvl_lr, inner_ci_level = .9)
...
library(ggeffects)
ggpredict(burg_median_income_lr, terms = "median_income") |> 
  plot()

```
```{r}
ggpredict(burg_median_income_real_lr, terms = "median_income") |> 
  plot()
```
```{r}
ggpredict(burg_median_income_lvl_lr, terms = "income_level") |> 
  plot()
```
...
leaflet(burglary_census_combo_latlong |> drop_na(long)) %>% 
  addTiles() %>% 
  addMarkers(
    ~long,
    ~lat,
    popup = ~as.character(`income_level`),
    label = ~as.character(`victim_number`),
...
predict(burg_median_income_lvl_lr, newdata = tibble(income_level = c("lowest", "low", "medium", "high"), population = c(100, 500, 1000, 5000)), type="response") 
...
highest_burgs_per_pop_pos <- highest_burgs_per_pop |>
  filter(median_income >= 0)
# highest_burgs_per_pop$TRACTCE[1]
...
# burgs_num_per_tract_real_pos
burgs_num_per_tract_real
highest_burgs_per_pop_pos
```
```{r}

burg_nums_pop_median_income <- merge(burgs_num_per_tract_real, highest_burgs_per_pop_pos, by.x=c("tract", "median_income", "population"), by.y=c("TRACTCE", "median_income", "population"), all.x=TRUE) 

# burg_median_income_lr <- glm(
#   num_burglaries ~ median_income,
#   data = burg_nums_pop_median_income,
#   family = 'poisson',
#   offset=log(population)
# )
# summary(burg_median_income_lr)
...
# burgs_num_per_tract_pop <- glm(
#   num_burglaries ~ median_income,
#   data = burg_nums_pop_median_income,
#   family = 'poisson',
#   offset=log(population)
# )
# summary(burgs_num_per_tract_pop)
```
...
# burgs_num_per_tract_pop <- glm(
#   high_burg_per_pop ~ median_income,
#   data = highest_burgs_per_pop_pos,
#   family = 'poisson',
#   offset=log(population)
# )
# summary(burgs_num_per_tract_pop)

# predict(highest_burgs_per_pop_pos, newdata = tibble(median_income, population =100), type="response") 
